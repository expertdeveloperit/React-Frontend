...
class Category extends Component {
  ...
  detectChange(value) {
    if (this.props.Category.viewquickCategories_Data.result) {
      if (this.props.Category.viewquickCategories_Data.result.length > 0) {
        const selectedData = this.props.Category.viewquickCategories_Data.result.filter(data => value.value == data.title);
  ...
  render() {
    ...
    if (this.props.Category.viewquickCategories_Data.result) {
      if (this.props.Category.viewquickCategories_Data.result.length > 0) {
        const my = this;
        const map = this.props.Category.viewquickCategories_Data.result.map((data) => {
