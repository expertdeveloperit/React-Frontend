...
class DragSortableList extends React.Component {
  ...
  componentDidMount() {
    ...
    interact(draggableChildrenSelector)
      .draggable({
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (moveTransitionDuration) {
      ...
      if (
        placeholderEl &&
        get(prevState, "placeholder.rank") &&
        get(prevState, "placeholder.rank") !==
          get(this.state, "placeholder.rank")
      ) {
        const itemsRefs = union(
          ["placeholder"],
          items.map(item => "item-" + item.id),
  ...
  _displayItem(item, type) {
    ...
    let style = {
      position: "relative",
      float:
        layoutType === "horizontal" || layoutType === "grid" ? "left" : "none",
    }
  ...
  _movePlaceholder(e) {
    ...
    children.forEach(child => {
      newPlaceholder = this._calculatePlaceholder(
        child,
        mouseX,
        mouseY,
        newPlaceholder,
